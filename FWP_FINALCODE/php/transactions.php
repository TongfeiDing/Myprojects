...
switch($action){
	...
	case 'makedeal':
		...
		break;
	case 'find':
		findTransactionbyID($conn,$userid);
		break;
	default:
...
function viewTransactionsofUser($conn,$userid){
	...
	$sql = "select * from Transactions inner join Items on Transactions.itemID = Items.itemID where Items.uploaderID = {$userID}";
...
function findTransactionbyID($conn,$userid){
	$responsecode = 0;
    $response = array();
